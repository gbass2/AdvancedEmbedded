...
// Function prototypes.
void setupADC(enum axisStates);                                                      // Setup the adc pin connected to the potentiometer.
...
void setupADC(enum axisStates select) {

    if(select == axisY) {
        P1SEL |= ACC_Y;                       // Set pin to analog.
        ADC10AE0 = ACC_Y;                     // Select channel A4
        ADC10CTL1 = INCH_5 + ADC10DIV_3;    // Select Channel A4, ADC10CLK/3
        ADC10CTL0 = ADC10SHT_3 + MSC + ADC10ON;
    }

    else if(select == axisX) {
        P1SEL |= ACC_X;                       // Set pin to analog.
        ADC10AE0 = ACC_X;                     // Select channel A5
        ADC10CTL1 = INCH_4 + ADC10DIV_3;    // Select Channel A5, ADC10CLK/3
        ADC10CTL0 = ADC10SHT_3 + MSC + ADC10ON;
    }
 ...
 * Description: Samples one of the A4-A5 analog pins and returns the digital value.
 * Input:       enum axisStates select
...
void calibrate() {
    // Read from the adc.
    xMedian = readAnalog(axisX);

    __delay_cycles(100000);

    yMedian = readAnalog(axisY);
}
